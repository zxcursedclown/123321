...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_search = tk.Button(toolbar, text='Поиск по названию', bg='#a5c4d9', bd=0,
                               compound=tk.TOP, height = 3 , width = 17,command=self.open_search_dialog)
        btn_search.pack(side=tk.TOP)

        btn_search = tk.Button(toolbar, text='Поиcк по цене', bg='#a5c4d9', bd=0,
                               compound=tk.TOP, height=3, width=17, command=self.open_search_dialog_price)
        btn_search.pack(side=tk.TOP)
    ...
    def search_pr(self, description):
        description = ('%' + description + '%',)
        self.db.c.execute('''SELECT * FROM accounting WHERE price LIKE ?''', description)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db.c.fetchall()]
    ...
    def open_search_dialog(self):
        Search()
    def open_search_dialog_price(self):
        SearchPR()
...
class SearchPR(tk.Toplevel):
    ...
    def init_search(self):
        ...
        btn_search.bind('<Button-1>', lambda event: self.view.search_pr(self.entry_search.get()))
...
class DB:
    def __init__(self):
        ...
        self.conn.commit()
        sql_query = "SELECT * FROM accounting "  # Пример параметризированного запроса
        cursor = self.conn.cursor()
        cursor.execute(sql_query, ())  # Передача значения переменной в execute()
        self.c.execute('''INSERT INTO accounting (description, category, price) VALUES (1, 2, 3) ''')
